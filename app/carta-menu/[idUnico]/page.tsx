...
export default function CartaMenuByIdPage() {
  ...
  useEffect(() => {
    const loadMenuById = async () => {
      console.log('≡ƒöì Cargando men├║ para ID:', idUnico);
      
      try {
        // Llamar a la API din├ímica con el ID ├║nico
        const response = await fetch(`/api/menu/${idUnico}`);
        const data = await response.json();
        
        if (data.success && data.menu) {
          console.log('Γ£à Men├║ cargado desde API:', data.menu);
        ...
        console.error('Γ¥î Error cargando men├║ desde API:', error);
        
        // Fallback a datos demo si falla la API
        console.log('ΓÜá∩╕Å Usando datos demo como fallback...');
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center text-white">
          <h1 className="text-2xl mb-4">ΓÜá∩╕Å Men├║ no encontrado</h1>
    ...
    <div className={`min-h-screen transition-colors duration-300 ${
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
          <h2 className="text-xl font-bold text-gray-900 mb-4">≡ƒôï Informaci├│n del Restaurante</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <p className="text-gray-600 mb-1">ID ├Ünico:</p>
        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          <h2 className="text-xl font-bold text-gray-900 mb-4">≡ƒì╜∩╕Å Categor├¡as del Men├║</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  ...
                  {category.items.length > 4 && (
                    <div className="text-gray-500 text-sm italic">
                      ... y {category.items.length - 4} productos m├ís
                    </div>
                  )}
                </div>
              </div>
